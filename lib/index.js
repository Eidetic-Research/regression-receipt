...
export function verifyRegression(input) {
    ...
    try {
        ...
        try {
            runGit(['worktree', 'remove', '--force', baseDir], cwd);
        }
        catch { /* best effort cleanup */ }
        try {
            runGit(['worktree', 'remove', '--force', headDir], cwd);
        }
        catch { /* best effort cleanup */ }
        fs.rmSync(tmp, { recursive: true, force: true });
    }
}
function isBugLike(input) {
    if (input.config.bug_labels.some((label) => input.labels.has(label.toLowerCase())))
        return true;
    if (input.config.bug_title_patterns.some((pattern) => matchesRegex(input.title, pattern)))
        return true;
